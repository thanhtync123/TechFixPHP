...
// ====================================================
// 🟡 API NỘI BỘ: LẤY THÔNG TIN 1 USER (TRẢ VỀ JSON)
// ====================================================
if (isset($_GET['get_user_json'])) {
    ...
    // Xóa mật khẩu khỏi JSON để bảo mật
    if($data) unset($data['password']); 
    
    echo json_encode($data);
    exit; // Dừng code tại đây, không load HTML bên dưới
}
// ====================================================
...
<script>
    ...
    // === HÀM XỬ LÝ EDIT KHÔNG RELOAD ===
    function loadUserForEdit(id) {
        // 1. Cuộn lên form
        document.getElementById('formCard').scrollIntoView({ behavior: 'smooth' });
        
        // 2. Đổi trạng thái form sang "Đang tải..."
        ...
        fetch('users.php?get_user_json=' + id)
            ...
            .then(data => {
                if (data) {
                    ...
                    // 5. Đổi giao diện Form sang chế độ "Cập nhật"
